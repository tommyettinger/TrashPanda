...
public class Scavenging {
    ...
    numberPattern =
            Pattern.compile("(?<!\\p{Jp})-?(?=[1-9]|0(?!\\d))\\d+(\\.\\d+)?([eE][+-]?\\d+)?");
    ...
    /**
     * Takes a String representing a number, reads in the number, takes a figurative shotgun to the bits, and
     * re-encodes to double.
     * @param original a String storing a double
     * @return a mangled version of the original double.
     */
    private static String shotgun(String original)
    {
        return String.valueOf(Double.longBitsToDouble(mix(Double.doubleToLongBits(Double.parseDouble(original)))));
    }

    /**
     * Takes a String representing a number that has been blown apart by {@link #shotgun(String)}, reads in the
     * number, reconstitutes the bits as if making a fur hat, and re-encodes to double.
     * @param original a String storing a double
     * @return a de-mangled version of the original double.
     */
    private static String hat(String original)
    {
        return String.valueOf(Double.longBitsToDouble(invMix(Double.doubleToLongBits(Double.parseDouble(original)))));
    }
    ...
    public static String scramble(String input, long seed)
    {
        StatefulRNG iRNG = new StatefulRNG(seed),
                ...
                oRNG = new StatefulRNG(nRNG.nextLong());
        ...
        segments = match.foundStrings();
        mixer = nRNG.randomOrdering(segments.size());
        match.setPosition(0);
        for(int i = 0; i < mixer.length; i++)
        {
            segments.set(i, shotgun(segments.get(i)));
        }
    ...
    public static String unscramble(String input, long seed)
    {
        ...
        for(int i = 0; i < mixer.length; i++)
        {
            segments.set(i, hat(segments.get(i)));
        }
        scram.reset(mixer, segments);
        return match.replaceAll(scram);
    }}
